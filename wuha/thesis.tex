...
$\\\bullet$ start with an initial velocity field $\overrightarrow{u}^{0}$ with the property ${\nabla}\cdot\overrightarrow{u} = 0$
$\\\bullet$ choose a time step $\Delta t$ 
$\\\bullet$ For time step n = 0, 1, 2, ...
\par$\bullet$  $\overrightarrow{u}^{A} = advect(\overrightarrow{u}^{n}, \overrightarrow{u}^{n}, \Delta t)$
 \par$\bullet$  add the extra force$\overrightarrow{u}^{B} = \overrightarrow{u}^{A} + \Delta t\overrightarrow{g}$
 \par$\bullet$  $\overrightarrow{u}^{n+1} =make\_incompressible(\Delta t, \overrightarrow{u}^{B})$


\section{Deep learning tasks}

There are also various ways of integrating deep learning model into the fluid simulation. 
