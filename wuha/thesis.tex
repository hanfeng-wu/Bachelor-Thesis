...
Then PDE models with machine learning started to be popular \cite{crutchfield1987equations,kevrekidis2003equation,brunton2016discovering}.
Then a lot of 

With the development of Phiflow \cite{holl2020learning}, Beside performing physical simulations, we are able to integrate different deep learning backends (Tensorflow and Pytorch), so that we can track the gradient of the vectors that are stored in different grid formats. With the help of Phiflow, \cite{um2020sol} used phiflow based deep learningl methods to reduce the numerical error when performing fluid simulations.
